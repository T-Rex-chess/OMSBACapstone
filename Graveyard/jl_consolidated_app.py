...
class DataLoader(BaseManager):
    ...
    # jlei: added a new methodto handle the monthly aggregation of variable ORDERDATE
    '''
    ex. 
    2/2004 -> 2
    ...
...
#jlei: aggregate by month
data_loader.aggregate_by_month()
print('Here is the row count of the processed dataframe: ',data_loader.row_count())
...
summ_stats_df.preprocess_data()
summ_stats_df.handle_missing_values()
summ_stats_df.aggregate_by_month() #jlei - aggregate by month
summary_stats = summ_stats_df.get_summary_stats()
print(summary_stats)
...
def bridge(data_set):
    ...
    if data_set == "Sales":
        ...
        data_loader = DataLoader(sales_data)
        data_loader.select_and_drop()
        data_loader.aggregate_by_month() #jlei - aggregate by month 
