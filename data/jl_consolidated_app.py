...
def guess():
    ...
    if request.method == 'POST':
        ...
        p = figure(
            ...
            background_fill_color=theme['chart_background']
        )
...
def display():
    ...
    if request.method == 'POST':
        ...
        # User guesses
        user_guesses = [
            float(request.form.get("g1")),
            float(request.form.get("g2")),
            float(request.form.get("g3")),
            float(request.form.get("g4")),
            float(request.form.get("g5"))
        ]
        ...
        <html lang="en">
            ...
            <body>
                ...
                {div}
                {script}
                <h2>{"You Won this Round! Good job predicting!" if user_mape <= regr_mape else "ML Wins this Round! Better luck next time."}</h2>
                <h3>Your MAPE: {user_mape}, ML's MAPE: {regr_mape}</h3>
                <p>Game is scored using Mean Absolute Percentage Error (MAPE). Higher MAPE = Less Accurate, Lower MAPE = More Accurate.</p>
                <form action="/guess" method="POST">
                    <button type="submit" class="btn-custom">{"Nice work, think you can do it again?" if user_mape <= regr_mape else "Play again? Think you can beat the machine?"}</button>
                </form>
                <form action="/" method="POST">
                    <button type="submit" class="btn-custom">Return to Homepage</button>
                </form>
            </body>
        </html>
        '''
